use music_streaming_blocklist_backend::{
    DatabaseConfig, initialize_database, db_health_check
};
...
async fn test_database_initialization() {
    ...
    if std::env::var("RUST_ENV").unwrap_or_else(|_| "development".to_string()) == "development" {
        assert!(user_count > 0, "Expected test users to be seeded in development");
    }
...
async fn test_database_health_check_functionality() {
    ...
    for i in 0..3 {
        ...
        assert!(health.response_time_ms >= 0, "Response time should be non-negative");
...
async fn test_migration_idempotency() {
    ...
    let pool1 = initialize_database(config.clone()).await.expect("First initialization failed");
    let pool2 = initialize_database(config).await.expect("Second initialization failed");
