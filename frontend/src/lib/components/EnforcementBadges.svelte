<script lang="ts">
  ...
  const platformStyles: Record<string, { connected: string; label: string }> = {
    spotify: {
...
{#if activePlatforms.length > 0}
  <div class="flex items-center gap-1" title="Enforced on: {activePlatforms.map(p => platformStyles[p]?.label).join(', ')}">
