...
impl OAuthErrorRecoveryService {
        ...
        while attempt <= self.config.max_retries {
            ...
            if !self.can_execute_operation(&provider).await {
                return Err(AppError::OAuth(OAuthError::ProviderUnavailable {
                    ...
                    estimated_recovery: Some(chrono::Utc::now() + chrono::Duration::seconds(self.config.circuit_breaker_timeout as i64)),
    ...
    fn calculate_retry_delay(&self, attempt: u32, oauth_error: &OAuthError) -> u64 {
        ...
        let jitter = (capped_delay as f64 * self.config.jitter_factor * rand::random::<f64>()) as u64;
    ...
    pub async fn get_provider_health(&self, provider: &OAuthProviderType) -> ProviderHealthStatus {
        let is_circuit_open = self.get_circuit_breaker_status(provider).await.unwrap_or(false);
        ...
        if is_circuit_open {
            ...
            ProviderHealthStatus::Degraded {
                reason: format!("High number of security violations: {}", security_violations),
            }
        } else {
        ...
        match oauth_error {
            OAuthError::ProviderNotConfigured { provider, missing_variables, .. } => {
                UserGuidance {
                    ...
                    actions: vec![
                        "Contact your system administrator to enable this authentication method".to_string(),
            ...
            OAuthError::InvalidConfiguration { provider, .. } => {
                UserGuidance {
                    title: format!("{} Authentication Error", provider),
                    message: format!("{} authentication is temporarily unavailable due to a configuration issue.", provider),
                    actions: vec![
            ...
            OAuthError::StateValidationFailed { .. } => {
                UserGuidance {
                    ...
                    actions: vec![
                        ...
                        "Make sure you're not using an old or bookmarked authentication link".to_string(),
                    ],
                    is_user_actionable: true,
                    contact_support: false,
                }
            }
            OAuthError::TokenRefreshFailed { provider, requires_reauth, .. } => {
                if *requires_reauth {
                    UserGuidance {
                        title: format!("{} Authentication Expired", provider),
                        message: format!("Your {} authentication has expired and needs to be renewed.", provider),
                        actions: vec![
                    ...
                    UserGuidance {
                        title: format!("{} Authentication Error", provider),
                        message: "There was a temporary issue refreshing your authentication.".to_string(),
                        actions: vec![
            ...
            OAuthError::InsufficientScopes { provider, required_scopes, .. } => {
                UserGuidance {
                    title: format!("{} Permissions Required", provider),
                    message: format!("Additional permissions are needed to complete this action with {}.", provider),
                    actions: vec![
                        format!("Re-authorize with {} to grant the required permissions", provider),
            ...
            OAuthError::RateLimitExceeded { provider, retry_after, .. } => {
                UserGuidance {
                    title: format!("{} Rate Limit", provider),
                    message: format!("Too many requests to {}. Please wait before trying again.", provider),
                    actions: vec![
            ...
            OAuthError::ProviderUnavailable { provider, estimated_recovery, .. } => {
                let recovery_message = if let Some(recovery_time) = estimated_recovery {
                    format!("Expected to be available again around {}", recovery_time.format("%H:%M"))
                } else {
            ...
            OAuthError::NetworkError { provider, .. } => {
                UserGuidance {
...
pub enum ProviderHealthStatus {
    ...
    Unavailable { reason: String, estimated_recovery: Option<chrono::DateTime<chrono::Utc>> },
}
...
mod tests {
    ...
    async fn test_retry_with_exponential_backoff() {
        ...
        let result = recovery_service.execute_with_recovery(
