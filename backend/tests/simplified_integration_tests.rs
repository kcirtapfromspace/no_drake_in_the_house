...
async fn test_apple_music_api_integration_flow() {
    ...
    assert_eq!(
        storefront_data["data"][0]["attributes"]["name"],
        "United States"
    );
...
async fn test_musicbrainz_api_integration() {
    ...
    let isni_relation = relations.iter().find(|rel| rel["type"] == "isni").unwrap();
    assert!(isni_relation["url"]["resource"]
        .as_str()
        .unwrap()
        .contains("isni.org"));
...
async fn test_all_integration_scenarios() {
    ...
    run_with_timeout(
        ...
        test_timeout,
        test_spotify_api_integration_flow(),
    )
    .await;
    run_with_timeout(
        "Apple Music API Integration",
        test_timeout,
        test_apple_music_api_integration_flow(),
    )
    .await;
    run_with_timeout(
        "MusicBrainz API Integration",
        test_timeout,
        test_musicbrainz_api_integration(),
    )
    .await;
