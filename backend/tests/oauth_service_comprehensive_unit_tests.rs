...
use backend::models::oauth::{
    OAuthAccount, OAuthFlowResponse, OAuthProviderType, OAuthState, OAuthTokenStatus, OAuthTokens,
    OAuthUserInfo, TokenExpirationStatus,
};
...
async fn test_oauth_state_manager_expired_state() {
    ...
    let result = state_manager.validate_and_consume_state(&state_token, &OAuthProviderType::Google);
    assert!(result.is_err());
}
...
async fn test_oauth_state_manager_invalid_token() {
    ...
    let result =
        state_manager.validate_and_consume_state("invalid_token", &OAuthProviderType::Google);
    assert!(result.is_err());
}
...
async fn test_oauth_token_encryption_performance() {
    ...
    assert!(
        encrypt_duration.as_millis() < 10,
        "Encryption took {}ms, should be under 10ms",
        encrypt_duration.as_millis()
    );
    assert!(
        decrypt_duration.as_millis() < 10,
        "Decryption took {}ms, should be under 10ms",
...
async fn test_mock_oauth_provider_get_user_info() {
    ...
    let result = provider.get_user_info("mock_access_token").await;
...
async fn test_mock_oauth_provider_failure_scenarios() {
    ...
    let result = provider
        .exchange_code("invalid_code", "state", "redirect")
        .await;
    assert!(result.is_err());
...
async fn test_mock_oauth_provider_refresh_token() {
    ...
    let result = provider.refresh_token("mock_refresh_token").await;
    ...
    assert!(tokens
        ...
        .as_ref()
        .unwrap()
        .starts_with("new_mock_refresh_token"));
}

#[tokio::test]
async fn test_mock_oauth_provider_with_delay() {
    let provider = MockOAuthProvider::new(OAuthProviderType::Google).with_delay(100); // 100ms delay
...
async fn test_oauth_flow_response_creation() {
    ...
    assert_eq!(
        flow_response.authorization_url,
        "https://example.com/oauth/authorize"
    );
    assert_eq!(flow_response.state, "test_state_123");
    assert_eq!(
        flow_response.code_verifier,
        Some("test_verifier_456".to_string())
    );
}
