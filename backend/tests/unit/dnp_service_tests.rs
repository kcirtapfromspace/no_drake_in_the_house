...
async fn test_add_artist_to_dnp_list(#[future] dnp_service: (DnpService, TestDatabase)) {
    ...
    assert_eq!(entry.tags, Some(vec!["test".to_string(), "rock".to_string()]));
...
async fn test_add_duplicate_artist_to_dnp_list(#[future] dnp_service: (DnpService, TestDatabase)) {
    ...
    service.add_to_dnp_list(user.id, request.clone()).await.unwrap();
...
async fn test_remove_from_dnp_list(#[future] dnp_service: (DnpService, TestDatabase)) {
    ...
    let removed = service.remove_from_dnp_list(user.id, artist.id).await.unwrap();
    assert!(removed);
...
async fn test_update_dnp_entry(#[future] dnp_service: (DnpService, TestDatabase)) {
    ...
    let updated_entry = service.update_dnp_entry(user.id, artist.id, update_request).await.unwrap();
    
    assert_eq!(updated_entry.tags, Some(vec!["updated".to_string(), "new".to_string()]));
