...
<div class="overflow-hidden rounded-lg hover:border-zinc-500 transition-all" style="background: #27272a; border: 2px solid #52525b;">
  <div class="p-6">
    <div class="flex items-center justify-between">
      <div class="flex items-center">
        ...
        {#if isSubscribed}
          <span class="ml-2 flex items-center px-2 py-0.5 rounded text-xs font-medium text-green-400 bg-green-400/15">
            Subscribed
          </span>
        {/if}
      </div>
    ...
    <div class="mt-3">
      <h4 class="text-xs font-medium text-zinc-400 uppercase tracking-wide">Criteria</h4>
    ...
    <div class="mt-4 flex items-center justify-between text-sm text-zinc-400">
      <div class="flex items-center space-x-4">
        <span>
          <svg aria-hidden="true" class="inline w-4 h-4 text-zinc-400 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="mt-4 text-xs text-zinc-500">
      ...
      {#if list.governance_url}
        • <a href={list.governance_url} target="_blank" rel="noopener noreferrer" class="text-rose-400 hover:text-rose-300 transition-colors">Governance</a>
      {/if}
    </div>
  </div>

  <div class="px-6 py-3" style="background: #3f3f46; border-top: 1px solid #52525b;">
    <div class="flex justify-between items-center">
      <button
        type="button"
        on:click={viewDetails}
        class="text-sm text-rose-400 hover:text-rose-300 font-medium transition-colors"
      >
        View Details
      </button>
      ...
      <button
        ...
        class="inline-flex items-center px-3 py-2 text-sm leading-4 font-medium rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 {isSubscribed
          ? 'text-red-400 hover:text-red-300 focus:ring-red-500'
          : 'text-rose-400 hover:text-rose-300 focus:ring-rose-500'}"
        style="background: {isSubscribed ? 'rgba(239, 68, 68, 0.15)' : 'rgba(244, 63, 94, 0.15)'}; border: 1px solid {isSubscribed ? 'rgba(239, 68, 68, 0.4)' : 'rgba(244, 63, 94, 0.4)'};"
      >
        {#if isSubscribed}
