    ...
    let response = app
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/v1/users/profile")
                .header(header::AUTHORIZATION, format!("Bearer {}", auth_response.access_token))
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
mod performance_tests {
    ...
    #[tokio::test]
    #[serial]
    async fn test_registration_endpoint_performance(#[future] test_app: (axum::Router, TestDatabase)) {
        let (app, _db) = test_app.await;
