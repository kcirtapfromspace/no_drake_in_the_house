...
mod common;
use common::{TestDatabase, init_test_tracing};
...
async fn test_successful_registration_with_auto_login() {
    ...
    assert!(!auth_response.access_token.is_empty(), "Access token should be provided");
    assert!(!auth_response.refresh_token.is_empty(), "Refresh token should be provided");
    
    // Verify user profile
    assert_eq!(auth_response.user.email, "test@example.com");
    assert!(!auth_response.user.email_verified, "Email should not be verified initially");
    assert!(!auth_response.user.totp_enabled, "TOTP should not be enabled initially");
    ...
    let audit_count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM audit_log WHERE action = 'user_registered'")
        .fetch_one(&test_db.pool)
        .await
        .unwrap();
...
async fn test_registration_with_auto_login_disabled() {
    ...
    assert!(result.is_ok(), "Registration should succeed even with auto-login disabled");
...
async fn test_registration_validation_errors() {
    ...
    assert!(result.is_err(), "Registration should fail with validation errors");
    
    match result.unwrap_err() {
        AppError::RegistrationValidationError { errors } => {
            // Verify all expected validation errors are present
            assert!(errors.iter().any(|e| e.field == "email"), "Should have email error");
            assert!(errors.iter().any(|e| e.field == "password"), "Should have password error");
            assert!(errors.iter().any(|e| e.field == "confirm_password"), "Should have confirm_password error");
            assert!(errors.iter().any(|e| e.field == "terms_accepted"), "Should have terms_accepted error");
            
            // Verify error codes
            assert!(errors.iter().any(|e| e.code == "EMAIL_INVALID_FORMAT"), "Should have email format error");
            assert!(errors.iter().any(|e| e.code == "PASSWORD_WEAK"), "Should have password weak error");
            assert!(errors.iter().any(|e| e.code == "PASSWORD_MISMATCH"), "Should have password mismatch error");
            assert!(errors.iter().any(|e| e.code == "TERMS_NOT_ACCEPTED"), "Should have terms not accepted error");
        }
    ...
    let user_count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM users")
        ...
        .await
        .unwrap();
    assert_eq!(user_count, 0, "No user should be created with validation errors");
...
async fn test_registration_duplicate_email() {
    ...
    assert!(result2.is_err(), "Second registration with same email should fail");
    ...
    let user_count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM users WHERE email = $1")
        ...
        .await
        .unwrap();
    assert_eq!(user_count, 1, "Only one user should exist with duplicate email");
...
async fn test_registration_database_transaction_integrity() {
    ...
    let user_count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM users WHERE email = $1")
        ...
        .await
        .unwrap();
    assert_eq!(user_count, 0, "No user should be created when validation fails");
    
    let audit_count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM audit_log WHERE old_subject_id LIKE '%transaction_test%'")
        .fetch_one(&test_db.pool)
        .await
        .unwrap();
    assert_eq!(audit_count, 0, "No audit log should be created when validation fails");
...
async fn test_registration_password_hashing() {
    ...
    let password_hash = sqlx::query_scalar::<_, Option<String>>("SELECT password_hash FROM users WHERE email = $1")
    ...
    assert_ne!(hash, "TestPassword123!", "Password should not be stored in plain text");
    ...
    if let Ok(cost) = parts[2].parse::<u32>() {
        assert!(cost >= 12, "Bcrypt cost should be at least 12, got {}", cost);
    }
...
async fn test_registration_audit_logging() {
    ...
    let audit_count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM audit_log WHERE action = 'user_registered'")
        ...
        .await
        .unwrap();
    assert!(audit_count > 0, "Registration should be logged in audit log");
...
async fn test_registration_token_generation() {
    ...
    let session_count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM user_sessions WHERE user_id = $1")
...
async fn test_registration_concurrent_requests() {
    ...
    let user_count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM users WHERE email = $1")
        ...
        .await
        .unwrap();
    assert_eq!(user_count, 1, "Only one user should be created despite concurrent requests");
...
async fn test_registration_edge_cases() {
    ...
    let result = auth_service.register(request).await;
    assert!(result.is_ok(), "Registration with long email should succeed");
    ...
    let result = auth_service.register(request).await;
    assert!(result.is_ok(), "Registration with special characters in password should succeed");
