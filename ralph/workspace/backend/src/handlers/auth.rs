...
pub async fn register_handler(
    ...
    let monitoring_service =
        RegistrationMonitoringService::new().map_err(|e| AppError::Internal {
...
pub async fn setup_2fa_handler(
    ...
    let response = state.auth_service.setup_2fa(user.id).await.map_err(|e| {
...
pub async fn verify_2fa_handler(
    ...
    let success = state
        .auth_service
        .verify_and_enable_2fa(user.id, &request.totp_code)
        .await
        .map_err(|e| {
...
pub async fn disable_2fa_handler(
    ...
    let success = state
        .auth_service
        .disable_2fa(user.id, &request.totp_code)
        .await
        .map_err(|e| {
