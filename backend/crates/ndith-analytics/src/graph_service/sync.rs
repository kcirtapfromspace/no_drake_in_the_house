//! Graph Sync Service
//!
//! Synchronizes data from PostgreSQL to the configured graph backend.
...
use crate::databases::{Collaboration, GraphArtist, SharedGraphStore};
...
impl GraphSyncService {
    ...
    pub fn new(graph: SharedGraphStore, pool: PgPool, config: GraphSyncConfig) -> Self {
        Self {
            config,
            graph,
            pool,
    ...
    async fn sync_artists(&self, _job: &mut SyncJob) -> Result<u32> {
        ...
        loop {
            ...
            for artist in artists {
                ...
                if let Err(e) = self.graph.upsert_artist(&graph_artist) {
    ...
    async fn sync_artists_since(&self, _job: &mut SyncJob, since: DateTime<Utc>) -> Result<u32> {
        ...
        for artist in artists {
            ...
            if self.graph.upsert_artist(&graph_artist).is_ok() {
                count += 1;
            }
        }
        ...
        match collabs {
            Ok(rows) => {
                for row in rows {
                    ...
                    if self.graph.add_collaboration(&collab).is_ok() {
                        count += 1;
                    }
                }
            }
    ...
    pub async fn sync_artist(&self, artist_id: Uuid) -> Result<()> {
        ...
        self.graph.upsert_artist(&graph_artist)?;
        Ok(())
    }
