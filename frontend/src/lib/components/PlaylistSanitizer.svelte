<script lang="ts">
  ...
  async function handleGrade() {
    ...
    await sanitizerActions.suggestReplacements(playlistInput.trim());
  }
...
<div class="sanitizer">
  <header class="sanitizer__header">
    ...
    <p class="sanitizer__subtitle">Grade, replace, and publish a clean version of any Spotify playlist</p>
  </header>
  ...
  {#if $sanitizerStore.step === 'grade'}
    <section class="sanitizer__section">
      <div class="sanitizer__input-row">
        <input
          type="text"
          class="sanitizer__input"
          placeholder="Paste Spotify playlist URL or ID..."
          bind:value={playlistInput}
          on:keydown={(e) => e.key === 'Enter' && handleGrade()}
        />
        <button
          type="button"
          class="brand-button brand-button--primary"
          on:click={handleGrade}
          disabled={$sanitizerStore.isGrading || $sanitizerStore.isSuggesting || !playlistInput.trim()}
        >
      ...
      {#if $sanitizerStore.currentGrade}
    ...
    <section class="sanitizer__section">
      {#if $sanitizerStore.isPublishing}
        <div class="publish-status">
          ...
          <p>Creating your sanitized playlist on Spotify...</p>
        </div>
