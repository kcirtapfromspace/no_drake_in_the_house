...
async fn test_rate_limiting_basic_functionality() {
    ...
    rate_limiter.record_success("spotify", response).await.unwrap();

    // Test optimal batch size calculation
    let batch_size = rate_limiter.get_optimal_batch_size("spotify", "remove_tracks").await.unwrap();
...
async fn test_optimal_batching() {
    ...
    let batches = rate_limiter.create_optimal_batches("spotify", "remove_tracks", items).await.unwrap();
