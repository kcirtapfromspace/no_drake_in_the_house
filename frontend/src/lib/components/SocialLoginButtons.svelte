<script lang="ts">
  ...
  function getProviderButtonClass(provider: string): string {
    const baseClass = "w-full flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed";
    
    switch (provider) {
      case 'google':
        return `${baseClass} bg-white text-gray-700 hover:bg-gray-50 focus:ring-2 focus:ring-offset-2 focus:ring-blue-500`;
      case 'apple':
        return `${baseClass} bg-black text-white hover:bg-gray-800 focus:ring-2 focus:ring-offset-2 focus:ring-gray-500`;
      case 'github':
        return `${baseClass} bg-gray-900 text-white hover:bg-gray-800 focus:ring-2 focus:ring-offset-2 focus:ring-gray-500`;
      default:
