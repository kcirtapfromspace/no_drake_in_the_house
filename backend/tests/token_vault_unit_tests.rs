...
async fn test_store_and_retrieve_token() {
    ...
    let store_result = vault_service.store_connection_tokens(&mut connection, store_request).await;
    ...
    assert_eq!(decrypted_tokens.refresh_token, Some("test_refresh_token_67890".to_string()));
...
async fn test_token_health_check() {
    ...
    vault_service.store_connection_tokens(&mut connection, store_request).await.unwrap();
...
async fn test_token_refresh() {
    ...
    let refresh_result = vault_service.refresh_connection_tokens(&mut connection, new_tokens).await;
    ...
    let retrieved_tokens = vault_service.get_decrypted_tokens(&connection).await.unwrap();
    assert_eq!(retrieved_tokens.access_token, "new_access_token");
    assert_eq!(retrieved_tokens.refresh_token, Some("new_refresh_token".to_string()));
}
...
async fn test_connection_management() {
    ...
    let retrieve_result = vault_service.get_user_connection(user_id, provider.clone()).await;
    assert!(retrieve_result.is_ok());
...
async fn test_connection_status_management() {
    ...
    vault_service.store_connection(connection.clone()).await.unwrap();
    
    // Test marking connection as error
    let error_result = vault_service.mark_connection_error(
        &mut connection,
...
async fn test_data_key_rotation() {
    ...
    let encrypted_v1 = vault_service.encrypt_token(test_token, user_id).await.unwrap();
    ...
    let encrypted_v2 = vault_service.encrypt_token("new_token_after_rotation", user_id).await.unwrap();
...
async fn test_bulk_token_operations() {
    ...
    let refresh_requests: Vec<_> = connections.iter().enumerate().map(|(i, conn)| {
        (conn.id, StoreTokenRequest {
...
async fn test_token_vault_error_handling() {
    ...
    let decrypt_result = vault_service.decrypt_token(&invalid_encrypted, user_id).await;
    ...
    let get_tokens_result = vault_service.get_decrypted_tokens(&nonexistent_connection).await;
