...
use music_streaming_blocklist_backend::{create_router, AppState, AuthService};
use std::sync::Arc;
...
async fn test_totp_endpoints_exist() {
    ...
    let setup_request = Request::builder()
        ...
        .body(Body::from(
            json!({"user_id": Uuid::new_v4().to_string()}).to_string(),
        ))
        .unwrap();
    ...
    let enable_request = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            json!({"user_id": Uuid::new_v4().to_string(), "totp_code": "123456"}).to_string(),
        ))
        .unwrap();
