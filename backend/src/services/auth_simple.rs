...
impl AuthService {
    ...
    pub async fn get_user(&self, user_id: Uuid) -> Result<User> {
        ...
        Ok(User {
            ...
            last_login: None,      // Default value since not in DB yet
            settings: serde_json::from_value(user.settings.unwrap_or_else(|| serde_json::json!({})))
                .unwrap_or_default(),
        })
    }
    ...
    pub async fn refresh_access_token(&self, _refresh_token: &str) -> Result<TokenPair> {
        ...
        Err(anyhow!("Refresh token functionality not implemented in simple auth service"))
    }

    pub async fn setup_totp(&self, _user_id: Uuid) -> Result<crate::models::TotpSetupResponse> {
        // Simplified implementation for testing
        Err(anyhow!("TOTP functionality not implemented in simple auth service"))
    }
    ...
    pub async fn create_session(&self, _user_id: Uuid, _device_info: String) -> Result<crate::models::Session> {
        // Simplified implementation for testing
        Err(anyhow!("Session management not implemented in simple auth service"))
    }
    ...
    pub async fn oauth_login(&self, _request: crate::models::OAuthLoginRequest) -> Result<TokenPair> {
        // Simplified implementation for testing
        Err(anyhow!("OAuth functionality not implemented in simple auth service"))
    }

    pub async fn link_oauth_provider(&self, _user_id: Uuid, _request: crate::models::OAuthLoginRequest) -> Result<()> {
        // Simplified implementation for testing
        Err(anyhow!("OAuth functionality not implemented in simple auth service"))
    }
}