...
async fn test_entity_resolution_service_creation() {
    ...
    assert_eq!(service.external_api_service.musicbrainz_client.base_url, "https://musicbrainz.org/ws/2");
    assert_eq!(service.external_api_service.isni_client.base_url, "https://isni.oclc.org/sru");
}
...
async fn test_add_artist_to_cache() {
    ...
    artist.id = artist_id;
    artist.external_ids.set_spotify_id("spotify_test_id".to_string());
    artist.external_ids.set_apple_id("apple_test_id".to_string());
    artist.aliases = vec![
        ArtistAlias {
    ...
    assert_eq!(retrieved_artist.external_ids.get_spotify_id(), Some("spotify_test_id"));
    assert_eq!(retrieved_artist.external_ids.get_apple_id(), Some("apple_test_id"));
...
async fn test_concurrent_artist_resolution() {
    ...
    artist1.external_ids.set_spotify_id("concurrent1_spotify".to_string());
    
    let artist2_id = Uuid::new_v4();
    let mut artist2 = Artist::new("Concurrent Artist 2".to_string());
    artist2.id = artist2_id;
    artist2.external_ids.set_spotify_id("concurrent2_spotify".to_string());
...
async fn test_musicbrainz_client_isni_extraction() {
    ...
    let test_cases = vec![
        ("https://isni.org/isni/0000000123456789", Some("0000000123456789")),
        ("https://isni.org/isni/0000000987654321", Some("0000000987654321")),
...
async fn test_artist_alias_confidence_scoring() {
    ...
    let high_conf_alias = retrieved.aliases.iter()
...
async fn test_external_id_mapping() {
    ...
    artist.id = artist_id;
    artist.external_ids.set_spotify_id("spotify_multi_id".to_string());
    artist.external_ids.set_apple_id("apple_multi_id".to_string());
    artist.external_ids.set_musicbrainz_id("mb_multi_id".to_string());
    artist.external_ids.set_isni_id("0000000123456789".to_string());
    
    service.add_artist(artist.clone()).await.unwrap();
    
    // Verify all external IDs are properly mapped
    let retrieved = service.get_artist_by_id(artist_id).await.unwrap().unwrap();
    assert_eq!(retrieved.external_ids.get_spotify_id(), Some("spotify_multi_id"));
    assert_eq!(retrieved.external_ids.get_apple_id(), Some("apple_multi_id"));
    assert_eq!(retrieved.external_ids.get_musicbrainz_id(), Some("mb_multi_id"));
    assert_eq!(retrieved.external_ids.get_isni_id(), Some("0000000123456789"));
