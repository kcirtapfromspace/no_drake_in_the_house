...
async fn test_jwt_token_validation() {
    ...
    let validation_result = auth_service.validate_access_token(&token_pair.access_token).await;
...
async fn test_refresh_token_flow() {
    ...
    // Test refresh token
    let refresh_result = auth_service.refresh_access_token(&initial_tokens.refresh_token).await;
...
async fn test_refresh_token_invalid() {
    ...
    let result = auth_service.refresh_access_token(invalid_refresh_token).await;
    assert!(result.is_err());
}
...
async fn test_totp_setup_and_verification() {
    ...
    let enable_result = auth_service.enable_totp(user.id, test_code.to_string()).await;
    // This might fail in the test since we're using a mock code, but we can test the structure
    
    // Test TOTP verification structure
    let verify_result = auth_service.verify_totp_code(user.id, test_code.to_string()).await;
...
async fn test_user_logout() {
    ...
    // Test logout
    let logout_result = auth_service.logout_user(user.id, &token_pair.refresh_token).await;
    assert!(logout_result.is_ok());
    
    // Test that refresh token is invalidated after logout
    let refresh_result = auth_service.refresh_access_token(&token_pair.refresh_token).await;
...
async fn test_session_management() {
    ...
    // Create session
    let session_result = auth_service.create_session(user.id, "test_device".to_string()).await;
...
async fn test_password_reset_flow() {
    ...
    let reset_request_result = auth_service.request_password_reset("reset@example.com".to_string()).await;
    ...
    let reset_result = auth_service.reset_password(reset_token, new_password.clone()).await;
...
async fn test_oauth_provider_management() {
    ...
    let link_result = auth_service.link_oauth_provider(user.id, link_request).await;
...
async fn test_token_expiration_handling() {
    ...
    let claims = auth_service.validate_access_token(&tokens.access_token).await.unwrap();
    assert!(claims.exp > Utc::now().timestamp());
