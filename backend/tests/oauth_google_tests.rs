...
async fn test_google_oauth_user_info_success() {
    ...
    assert_eq!(user_info.avatar_url, Some("https://lh3.googleusercontent.com/avatar.jpg".to_string()));
...
async fn test_google_oauth_initiate_flow() {
    ...
    assert!(flow_response.authorization_url.contains("accounts.google.com/o/oauth2/v2/auth"));
    ...
    assert!(flow_response.authorization_url.contains("scope=openid%20email%20profile"));
    ...
    assert!(flow_response.authorization_url.contains("include_granted_scopes=true"));
