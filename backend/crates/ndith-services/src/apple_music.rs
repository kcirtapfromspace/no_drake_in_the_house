...
impl AppleMusicService {
    ...
    ///
    /// Note: Apple Music user tokens (Music User Tokens) have an approximate 6-month expiry.
    /// Unlike OAuth refresh tokens, they cannot be renewed via API - users must re-authenticate
    /// through MusicKit JS when the token expires.
    pub async fn create_connection(
        ...
        // A successful HTTP transport is not enough: we require a 2xx Apple response.
        ...
        if !test_response.status().is_success() {
            ...
            if Self::is_cloud_library_privilege_error(status, &error_text) {
                return Err(anyhow!(
                    "Apple Music library access is unavailable for this account. Enable Sync Library and verify your Apple Music subscription."
                ));
            }

            if error_text.is_empty() {
                return Err(anyhow!(
                    "Invalid Apple Music user token (status {})",
                    status
                ));
            }
        ...
        // Apple Music user tokens expire after approximately 6 months
        // We set the expiry to 6 months from now to track when re-authentication is needed
        let expires_at = Some(Utc::now() + chrono::Duration::days(180));

        // Store the connection using the token vault
        let store_request = ndith_core::models::StoreTokenRequest {
            user_id,
    ...
    ///
    /// For Apple Music, this validates the token by making a test API call and checks
    /// if the token is approaching its 6-month expiry. Since Apple Music tokens cannot
    /// be refreshed via API, `needs_refresh` being true means re-authentication is needed.
    pub async fn check_token_health(&self, connection: &Connection) -> Result<TokenHealthCheck> {
        ...
        let is_valid = match response {
            ...
            Ok(response) => {
                ...
                validation_error = Some(if error_text.is_empty() {
            ...
            Err(error) => {
                validation_error = Some(format!("Token validation request failed: {}", error));
                false
            }
        };

        // Check if token is approaching expiry (within 7 days)
        // Apple Music tokens can't be refreshed - user must re-authenticate
        let needs_refresh = if let Some(expires_at) = connection.expires_at {
            let warning_threshold = Utc::now() + chrono::Duration::days(7);
        ...
        Ok(TokenHealthCheck {
            ...
            error_message: if !is_valid {
                ...
                Some("Apple Music token expiring soon. Re-authentication required.".to_string())
            } else {
                None
            },
            checked_at: Utc::now(),
            ...
            if !response.status().is_success() {
                ...
                if Self::is_cloud_library_privilege_error(status, &error_text) {
                    tracing::warn!(
                        connection_id = %connection.id,
                        status = %status,
                        "Apple Music playlist access unavailable (CloudLibrary privilege missing); continuing without playlists"
                    );
                    return Ok(Vec::new());
                }
                return Err(anyhow!(
    ...
    fn is_cloud_library_privilege_error(status: StatusCode, response_body: &str) -> bool {
    ...
    async fn wait_for_rate_limit(&self) -> Result<()> {
        ...
        if let Some(rate_limit) = rate_limits.get("global") {
            if rate_limit.remaining == 0 && Utc::now() < rate_limit.reset_at {
                let derived_retry_after = (rate_limit.reset_at - Utc::now()).num_seconds().max(1);
                let retry_after = rate_limit.retry_after.unwrap_or(derived_retry_after as u32);
                return Err(anyhow!("Rate limited, retry after {} seconds", retry_after));
            }
        }
        Ok(())
