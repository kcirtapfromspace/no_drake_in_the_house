...
async fn test_circuit_breaker_concurrent_access() {
    ...
    let cb = Arc::new(Mutex::new(CircuitBreaker::new(2, Duration::from_millis(100))));
    let mut handles = vec![];
...
fn test_basic_model_creation() {
    ...
    // Test User creation
    let user = User::new("test@example.com".to_string(), Some("password_hash".to_string()));
...
fn test_artist_alias_operations() {
    ...
    let high_confidence_aliases: Vec<_> = artist.aliases.iter()
    ...
    let mb_aliases: Vec<_> = artist.aliases.iter()
