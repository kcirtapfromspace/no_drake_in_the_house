...
export async function maybeHandleConvexRoute<T = unknown>(
  ...
  try {
    ...
    // =============================================
    // Community Lists (Phase 1)
    // =============================================

    if (method === 'GET' && pathname === '/api/v1/community/lists') {
      const result = await convexQuery<any>(anyApi.community.list, {});
      return ok(result) as BridgedApiResponse<T>;
    }

    if (method === 'POST' && pathname === '/api/v1/community/lists') {
      const result = await convexMutation<any>(anyApi.community.create, {
        name: data?.name,
        description: data?.description,
        criteria: data?.criteria,
        governanceUrl: data?.governance_url,
    ...
    if (method === 'GET' && pathname === '/api/v1/community/subscriptions') {
      const result = await convexQuery<any>(anyApi.community.subscriptions, {});
      return ok(result) as BridgedApiResponse<T>;
    }

    const communityListArtistsMatch = matchPath(
      pathname,
      /^\/api\/v1\/community\/lists\/([^/]+)\/artists$/,
    );
    if (communityListArtistsMatch && method === 'GET') {
      const listId = parseId(communityListArtistsMatch[1]);
      const result = await convexQuery<any>(anyApi.community.listArtists, { listId });
      return ok(result) as BridgedApiResponse<T>;
    }

    const communityImpactMatch = matchPath(
      pathname,
      /^\/api\/v1\/community\/lists\/([^/]+)\/impact$/,
    );
    if (communityImpactMatch && method === 'GET') {
      const listId = parseId(communityImpactMatch[1]);
      const result = await convexQuery<any>(anyApi.community.impact, { listId });
      return ok(result) as BridgedApiResponse<T>;
    }

    const communitySubscribeMatch = matchPath(
      pathname,
      /^\/api\/v1\/community\/lists\/([^/]+)\/subscribe$/,
    );
    if (communitySubscribeMatch && method === 'POST') {
      const listId = parseId(communitySubscribeMatch[1]);
      const result = await convexMutation<any>(anyApi.community.subscribe, {
        listId,
        versionPinned: data?.version_pinned,
        autoUpdate: data?.auto_update,
      });
      return ok(result) as BridgedApiResponse<T>;
    }

    const communityUnsubscribeMatch = matchPath(
      pathname,
      /^\/api\/v1\/community\/lists\/([^/]+)\/unsubscribe$/,
    );
    if (communityUnsubscribeMatch && method === 'POST') {
      const listId = parseId(communityUnsubscribeMatch[1]);
      const result = await convexMutation<any>(anyApi.community.unsubscribe, { listId });
      return ok(result) as BridgedApiResponse<T>;
    }

    const communitySubUpdateMatch = matchPath(
      pathname,
      /^\/api\/v1\/community\/lists\/([^/]+)\/subscription$/,
    );
    if (communitySubUpdateMatch && method === 'PUT') {
      const listId = parseId(communitySubUpdateMatch[1]);
      const result = await convexMutation<any>(anyApi.community.updateSubscription, {
        listId,
        versionPinned: data?.version_pinned,
        autoUpdate: data?.auto_update,
      });
