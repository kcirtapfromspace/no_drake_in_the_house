...
impl EmbeddingGenerator {
    ...
    pub async fn embed_article(
        ...
        let text_preview = preview_text(&text, 100);
    ...
    fn truncate_text(&self, text: &str) -> String {
        if text.len() <= self.config.max_length {
            ...
            let truncated = prefix_at_char_boundary(text, self.config.max_length);
...
fn preview_text(text: &str, max_len: usize) -> String {
    if text.len() > max_len {
        format!("{}...", prefix_at_char_boundary(text, max_len))
    } else {
        text.to_string()
    }
}

fn prefix_at_char_boundary(text: &str, max_len: usize) -> &str {
...
mod tests {
    ...
    #[test]
    fn test_truncate_text_handles_unicode_boundary() {
        let config = EmbeddingConfig {
            max_length: 16,
            ..Default::default()
        };
        ...
        let text = "Artist \u{a0}headline with emoji 🎵";
        let truncated = generator.truncate_text(text);
    ...
    #[test]
    fn test_preview_text_handles_unicode_boundary() {
        let text = "‘Sinners’ Stars Recreate the ‘Pierce the Veil’ Sequence";
        let preview = preview_text(text, 24);
        let prefix = preview.trim_end_matches("...");
