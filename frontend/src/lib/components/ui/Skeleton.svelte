...
{#if variant === 'text'}
  <div class="space-y-2" style="width: {width};">
    {#each Array(lines) as _, i}
      <div
        class="rounded h-4 {animate ? 'animate-pulse' : ''}"
...
{:else if variant === 'card'}
  ...
  >
    <div class="flex gap-4">
      <div
        class="rounded-lg flex-shrink-0 {animate ? 'animate-pulse' : ''}"
        style="background: #3f3f46; width: 80px; height: 80px;"
      ></div>
      <div class="flex-1 space-y-3">
        <div
          class="h-5 rounded {animate ? 'animate-pulse' : ''}"
          style="background: #3f3f46; width: 60%;"
        ></div>
        <div
          class="h-4 rounded {animate ? 'animate-pulse' : ''}"
          style="background: #52525b; width: 40%;"
        ></div>
        <div
          class="h-3 rounded {animate ? 'animate-pulse' : ''}"
