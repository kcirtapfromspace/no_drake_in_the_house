use music_streaming_blocklist_backend::{
    initialize_database, models::*, services::AuthService, DatabaseConfig, DnpListService,
};
use uuid::Uuid;
...
async fn test_dnp_service_integration_fixed() {
    ...
    let artist_id = dnp_service
        .create_or_find_artist("Test Artist", None)
        .await
        .unwrap();

    let dnp_entry = dnp_service
        .add_artist_to_dnp_list(
            user.id,
            artist_id,
...
async fn test_complete_user_workflow_simplified() {
    ...
    let artist_id = dnp_service
        .create_or_find_artist("Drake", None)
        .await
        .unwrap();
    let _dnp_entry = dnp_service
        .add_artist_to_dnp_list(
            user.id,
            artist_id,
    ...
    let exported_json = dnp_service
        .export_dnp_list(user.id, ImportFormat::Json)
        .await
        .unwrap();
    ...
    let updated_entry = dnp_service
        .update_dnp_entry(user.id, artist_id, update_request)
        .await
        .unwrap();
    ...
    dnp_service
        .remove_artist_from_dnp_list(user.id, artist_id)
        .await
        .unwrap();
