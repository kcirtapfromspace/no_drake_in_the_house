...
use crate::error::oauth::{OAuthError, parse_provider_error};
...
impl GoogleOAuthProvider {
    ...
    pub async fn test_configuration(&self) -> Result<()> {
        ...
        let response = self.base.client
            ...
            .send()
            .await
            .map_err(|e| AppError::OAuth(OAuthError::NetworkError {
        ...
        let discovery_doc: serde_json::Value = response.json().await
            .map_err(|e| AppError::OAuth(OAuthError::ProviderError {
                provider: OAuthProviderType::Google,
        ...
        let expected_auth_endpoint = discovery_doc["authorization_endpoint"].as_str()
            .ok_or_else(|| AppError::ExternalServiceError("Missing authorization_endpoint in Google discovery document".to_string()))?;
        let expected_token_endpoint = discovery_doc["token_endpoint"].as_str()
            .ok_or_else(|| AppError::ExternalServiceError("Missing token_endpoint in Google discovery document".to_string()))?;
        let expected_userinfo_endpoint = discovery_doc["userinfo_endpoint"].as_str()
            .ok_or_else(|| AppError::ExternalServiceError("Missing userinfo_endpoint in Google discovery document".to_string()))?;
    ...
    fn parse_user_info(&self, user_data: Value) -> Result<OAuthUserInfo> {
        ...
        if let Some(hd) = user_data["hd"].as_str() {
            provider_data.insert("hosted_domain".to_string(), serde_json::Value::String(hd.to_string()));
        }
        if let Some(link) = user_data["link"].as_str() {
            provider_data.insert("profile_link".to_string(), serde_json::Value::String(link.to_string()));
        }
...
impl OAuthProvider for GoogleOAuthProvider {
    ...
    async fn exchange_code(&self, code: &str, _state: &str, redirect_uri: &str) -> Result<OAuthTokens> {
        ...
        let token_response: serde_json::Value = response.json().await
            .map_err(|e| AppError::ExternalServiceError(format!("Failed to parse Google token response: {}", e)))?;
        ...
        if id_token.is_none() {
            tracing::warn!("Google did not return an ID token, OpenID Connect features may not work");
        }
    ...
    async fn get_user_info(&self, access_token: &str) -> Result<OAuthUserInfo> {
        ...
        if !response.status().is_success() {
            ...
            return Err(AppError::OAuth(OAuthError::UserInfoRetrievalFailed {
                provider: OAuthProviderType::Google,
                reason: format!("Google user info request failed with status {}: {}", status, error_text),
                missing_scopes: vec![],
            }));
        }

        let user_data: serde_json::Value = response.json().await
            .map_err(|e| AppError::ExternalServiceError(format!("Failed to parse Google user info response: {}", e)))?;
    ...
    async fn refresh_token(&self, refresh_token: &str) -> Result<OAuthTokens> {
        ...
        if !response.status().is_success() {
            ...
            if let Ok(error_json) = serde_json::from_str::<serde_json::Value>(&error_text) {
                ...
                match error_code {
                    ...
                    _ => {
                        return Err(AppError::OAuth(OAuthError::TokenRefreshFailed {
                            provider: OAuthProviderType::Google,
                            reason: format!("Token refresh failed ({}): {}", error_code, error_description),
            ...
            return Err(AppError::OAuth(OAuthError::TokenRefreshFailed {
                provider: OAuthProviderType::Google,
                reason: format!("Google token refresh failed with status {}: {}", status, error_text),
                requires_reauth: false,
            }));
        }

        let token_response: serde_json::Value = response.json().await
            .map_err(|e| AppError::ExternalServiceError(format!("Failed to parse Google refresh response: {}", e)))?;
    ...
    async fn revoke_token(&self, token: &str) -> Result<()> {
        if let Some(revoke_endpoint) = &self.base.revoke_endpoint {
            ...
            let response = self.base.client
                ...
                .send()
                .await
                .map_err(|e| AppError::ExternalServiceError(format!("Google token revocation request failed: {}", e)))?;
    ...
    fn validate_config(&self) -> Result<()> {
        ...
        if !is_dev_mode && !self.base.config.client_id.ends_with(".apps.googleusercontent.com") {
            return Err(AppError::ConfigurationError {
                message: "Google OAuth client_id must end with '.apps.googleusercontent.com'".to_string(),
            });
        }
...
impl GoogleOAuthService {
    ...
    pub async fn validate_id_token(&self, id_token: &str) -> Result<Value> {
        ...
        let validation_url = format!("https://oauth2.googleapis.com/tokeninfo?id_token={}", id_token);
        
        let response = self.provider.base.client
            .get(&validation_url)
            .send()
            .await
            .map_err(|e| AppError::ExternalServiceError(format!("ID token validation request failed: {}", e)))?;
        ...
        let token_info: Value = response.json().await
            .map_err(|e| AppError::ExternalServiceError(format!("Failed to parse token validation response: {}", e)))?;
...
mod tests {
    ...
    async fn test_google_initiate_flow() {
        ...
        assert!(flow_response.authorization_url.contains("accounts.google.com"));
        ...
        assert!(flow_response.authorization_url.contains("access_type=offline"));
    ...
    fn test_google_parse_user_info() {
        ...
        assert_eq!(user_info.avatar_url, Some("https://example.com/avatar.jpg".to_string()));
        ...
        assert_eq!(
            ...
            Some(&serde_json::Value::String("https://plus.google.com/123456789".to_string()))
        );
    }
    ...
    fn test_google_oauth_service() {
        ...
        assert_eq!(service.provider().provider_type(), OAuthProviderType::Google);
    }
    ...
    async fn test_google_oauth_service_with_options() {
        ...
        assert!(flow_response.authorization_url.contains("login_hint=user%40example.com"));
