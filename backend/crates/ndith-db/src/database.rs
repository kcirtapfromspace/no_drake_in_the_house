...
pub async fn run_migrations(pool: &PgPool) -> Result<(), sqlx::Error> {
    ...
    // Prefer runtime migration files so local/UTM-built binaries and runtime images
    // always execute the same migration set.
    let migration_paths = ["/app/migrations", "./migrations", "../migrations"];
    let migration_path = migration_paths
        .iter()
        .find(|p| Path::new(p).is_dir())
        .ok_or_else(|| {
            sqlx::Error::Configuration(
                format!(
                    "No migrations directory found (checked: {})",
    ...
    info!("Using migration directory: {}", migration_path);
