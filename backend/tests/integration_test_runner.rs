...
impl IntegrationTestRunner {
    ...
    pub async fn run_provider_sandbox_tests(&self) -> TestSuiteResult {
        ...
        let spotify_result =
            timeout(Duration::from_secs(30), self.run_spotify_sandbox_tests()).await;
    ...
    async fn run_spotify_sandbox_tests(
        &self,
    ...
    async fn run_apple_music_sandbox_tests(
        &self,
    ) -> Result<Vec<TestResult>, Box<dyn std::error::Error>> {
        let mut results = Vec::new();
    ...
    pub async fn run_end_to_end_tests(&self) -> TestSuiteResult {
        ...
        let test_cases = vec![
            (
                ...
                self.test_complete_user_onboarding(),
            ),
            (
                "Community List Subscription",
                self.test_community_list_subscription(),
            ),
            (
                "Multi-Platform Enforcement",
                self.test_multi_platform_enforcement(),
            ),
    ...
    pub async fn run_contract_tests(&self) -> TestSuiteResult {
        ...
        let test_cases = vec![
            ...
            (
                "Apple Music API Contracts",
                self.test_apple_music_api_contracts(),
            ),
            (
                "MusicBrainz API Contracts",
                self.test_musicbrainz_api_contracts(),
            ),
    ...
    pub async fn run_browser_extension_tests(&self) -> TestSuiteResult {
        ...
        let test_cases = vec![
            ...
            (
                ...
                self.test_extension_bloom_filter_performance(),
            ),
            ("Auto-Skip Functionality", self.test_extension_auto_skip()),
            (
                "Offline Functionality",
                self.test_extension_offline_functionality(),
            ),
            (
                "Selector Resilience",
                self.test_extension_selector_resilience(),
            ),
            (
                "Performance Under Load",
                self.test_extension_performance_under_load(),
            ),
...
impl TestSuiteResult {
    ...
    pub fn print_summary(&self) {
        ...
        println!(
            "{}: {}/{} passed ({:.1}%)",
            self.name, passed, total, success_rate
        );
    }
