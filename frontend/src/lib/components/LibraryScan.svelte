...
<div class="min-h-screen" style="background: #18181b;">
  ...
  <div class="max-w-4xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
    ...
    {#if !scanComplete && !isScanning}
      <!-- Start scan prompt -->
      <div class="rounded-2xl shadow-lg p-8 text-center" style="background: #27272a; border: 2px solid #52525b;">
        <div class="w-20 h-20 bg-purple-900/30 rounded-full flex items-center justify-center mx-auto mb-6">
        ...
        <div class="rounded-xl p-6 mb-8 text-left max-w-md mx-auto" style="background: #3f3f46;">
          <h3 class="font-medium text-white mb-3">Before you scan:</h3>
    ...
    {:else if isScanning}
      ...
      <div class="rounded-2xl shadow-lg p-8 text-center" style="background: #27272a; border: 2px solid #52525b;">
        <div class="w-20 h-20 bg-purple-900/30 rounded-full flex items-center justify-center mx-auto mb-6 animate-pulse">
          <svg class="w-10 h-10 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
        </div>
        ...
        <div class="max-w-md mx-auto">
          <div class="h-4 rounded-full overflow-hidden" style="background: #3f3f46;">
            <div
              class="h-full bg-purple-600 transition-all duration-300"
              style="width: {scanProgress}%"
            ></div>
          </div>
          <p class="text-sm text-zinc-400 mt-2">{scanProgress}% complete</p>
        </div>
      </div>
    ...
    {:else}
      ...
      <div class="space-y-6">
        <!-- Summary card -->
        <div class="rounded-2xl shadow-lg p-6" style="background: #27272a; border: 2px solid #52525b;">
          ...
          {#if $scanStats}
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              <div class="rounded-xl p-4 text-center" style="background: #3f3f46;">
                <div class="text-2xl font-bold text-white">{$scanStats.totalTracks.toLocaleString()}</div>
                <div class="text-sm text-zinc-400">Total Tracks</div>
              </div>
              <div class="rounded-xl p-4 text-center" style="background: #3f3f46;">
                <div class="text-2xl font-bold text-white">{$scanStats.totalArtists}</div>
                <div class="text-sm text-zinc-400">Artists</div>
              </div>
              <div class="bg-red-900/30 rounded-xl p-4 text-center">
                <div class="text-2xl font-bold text-red-400">{$scanStats.flaggedArtists}</div>
                <div class="text-sm text-red-400">Flagged Artists</div>
              </div>
              <div class="bg-red-900/30 rounded-xl p-4 text-center">
                <div class="text-2xl font-bold text-red-400">{$scanStats.flaggedTracks}</div>
                <div class="text-sm text-red-400">Flagged Tracks</div>
              </div>
            </div>
          {/if}
          ...
          {#if $flaggedArtists.length > 0}
            <div class="flex items-center gap-4 text-sm flex-wrap">
              <span class="text-zinc-400">By severity:</span>
        ...
        {#if $flaggedArtists.length > 0}
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold text-white">Flagged Artists</h3>
            <button
              type="button"
              on:click={blockAll}
          ...
          <div class="space-y-4">
            {#each $flaggedArtists as artist}
              <div class="rounded-2xl shadow-sm overflow-hidden" style="background: #27272a; border: 2px solid #52525b;">
                <div class="p-5">
                  <div class="flex items-start justify-between mb-4">
                    <div class="flex items-center">
                      <div class="w-12 h-12 rounded-full flex items-center justify-center text-xl mr-4" style="background: #3f3f46;">
                  ...
                  <div class="flex gap-3">
                    ...
                    <button
                      ...
                      class="px-4 py-2.5 text-zinc-300 font-medium rounded-xl transition-colors"
        ...
        {:else}
          <!-- Clean library -->
          <div class="bg-green-900/30 rounded-2xl p-8 text-center">
    ...
    <!-- Import prompt -->
    {#if !scanComplete}
      <div class="mt-8 bg-blue-900/30 rounded-2xl p-6">
        <div class="flex items-start">
          <div class="w-12 h-12 bg-blue-900/50 rounded-full flex items-center justify-center mr-4 flex-shrink-0">
            <svg class="w-6 h-6 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
            </svg>
          </div>
          <div>
            <h4 class="font-medium text-blue-300 mb-1">Import Your Music Library</h4>
            ...
            <button
              ...
              class="text-sm text-blue-400 font-medium hover:text-blue-300"
            >
              Go to Connections
            </button>
          </div>
    ...
    <div class="mt-8 rounded-2xl p-6" style="background: #3f3f46;">
      <div class="flex items-start">
        ...
        <div>
          <h4 class="font-medium text-white mb-1">About Our Evidence Database</h4>
          ...
          <button
            ...
            class="text-sm text-indigo-400 font-medium hover:text-indigo-300"
          >
