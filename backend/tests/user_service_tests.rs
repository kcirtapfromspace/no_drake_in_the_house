...
use music_streaming_blocklist_backend::services::user::{
    UserProfile, UpdateUserProfileRequest, AccountDeletionRequest
};
use uuid::Uuid;
use serde_json::json;
...
async fn test_user_service_basic_functionality() {
    ...
    let db_pool = initialize_database(db_config).await.expect("Failed to initialize database");
    ...
    let profile = user_service.get_profile(user_id).await.expect("Failed to get profile");
    ...
    let updated_profile = user_service.update_profile(user_id, update_request).await.expect("Failed to update profile");
    assert_eq!(updated_profile.email, "newemail@example.com");
    assert_eq!(updated_profile.settings.privacy_mode, true);
    
    // Test data export
    let export = user_service.export_user_data(user_id).await.expect("Failed to export data");
    ...
    let deletion_result = user_service.delete_account(user_id, deletion_request).await.expect("Failed to delete account");
...
async fn test_user_settings_update() {
    ...
    let updated_profile = user_service.update_profile(user_id, update_request).await.expect("Failed to update settings");
