...
impl OAuthTokenEncryption {
    ...
    pub fn new() -> Result<Self> {
        let key_base64 = env::var("OAUTH_ENCRYPTION_KEY")
        ...
        if key_bytes.len() != 32 {
            return Err(AppError::ConfigurationError { 
                message: "OAUTH_ENCRYPTION_KEY must be 32 bytes (256 bits) when base64 decoded".to_string() 
            });
        }
    ...
    pub fn encrypt_token(&self, token: &str) -> Result<Vec<u8>> {
        ...
        // Encrypt the token
        let ciphertext = self.current_cipher
        ...
        let mut encrypted_data = Vec::with_capacity(1 + 1 + key_id_bytes.len() + 12 + ciphertext.len());
    ...
    pub async fn decrypt_token(&self, encrypted_data: &[u8]) -> Result<String> {
        if encrypted_data.is_empty() {
            return Err(AppError::EncryptionError("Encrypted data is empty".to_string()));
        }
        ...
        if encrypted_data[0] == 1 {
            ...
            if encrypted_data.len() < 3 {
                return Err(AppError::EncryptionError("Versioned encrypted data too short".to_string()));
            }

            let key_id_len = encrypted_data[1] as usize;
            if encrypted_data.len() < 2 + key_id_len + 12 {
                return Err(AppError::EncryptionError("Versioned encrypted data incomplete".to_string()));
            }

            let key_id = String::from_utf8(encrypted_data[2..2 + key_id_len].to_vec())
                .map_err(|e| AppError::EncryptionError(format!("Invalid key ID in encrypted data: {}", e)))?;
            ...
            if key_id == self.current_key_id {
                let plaintext = self.current_cipher
                    .decrypt(nonce, ciphertext)
                    .map_err(|e| AppError::EncryptionError(format!("Token decryption failed with current key: {}", e)))?;

                return String::from_utf8(plaintext)
                    .map_err(|e| AppError::EncryptionError(format!("Decrypted token is not valid UTF-8: {}", e)));
            }
            ...
            if let Some(cipher) = historical_ciphers.get(&key_id) {
                let plaintext = cipher
                    .decrypt(nonce, ciphertext)
                    .map_err(|e| AppError::EncryptionError(format!("Token decryption failed with historical key {}: {}", key_id, e)))?;

                return String::from_utf8(plaintext)
                    .map_err(|e| AppError::EncryptionError(format!("Decrypted token is not valid UTF-8: {}", e)));
            }

            return Err(AppError::EncryptionError(format!("No key found for key ID: {}", key_id)));
        } else {
            // Legacy format (version 0): [nonce(12)] + [ciphertext]
            if encrypted_data.len() < 12 {
                return Err(AppError::EncryptionError("Legacy encrypted data too short to contain nonce".to_string()));
            }
            ...
            for (key_id, cipher) in historical_ciphers.iter() {
                if let Ok(plaintext) = cipher.decrypt(nonce, ciphertext) {
                    if let Ok(token) = String::from_utf8(plaintext) {
                        tracing::debug!("Successfully decrypted legacy token with historical key: {}", key_id);
                        return Ok(token);
                    }
                }
            }

            return Err(AppError::EncryptionError("Token decryption failed with all available keys".to_string()));
        }
    }

    /// Encrypt multiple tokens (access and refresh)
    pub fn encrypt_token_pair(&self, access_token: &str, refresh_token: Option<&str>) -> Result<(Vec<u8>, Option<Vec<u8>>)> {
    ...
    pub async fn decrypt_token_pair(&self, encrypted_access: &[u8], encrypted_refresh: Option<&[u8]>) -> Result<(String, Option<String>)> {
    ...
    pub async fn rotate_key(&mut self, new_key: &[u8]) -> Result<()> {
        ...
        
        {
            ...
            if historical_ciphers.len() > self.key_rotation_config.max_historical_keys {
                ...
                let keys_to_remove = historical_ciphers.len() - self.key_rotation_config.max_historical_keys;
                let keys: Vec<String> = historical_ciphers.keys().take(keys_to_remove).cloned().collect();
                for key in keys {
    ...
    pub fn needs_key_rotation(&self) -> bool {
        match self.key_rotation_config.last_rotation {
            Some(last_rotation) => {
                let rotation_threshold = last_rotation + Duration::days(self.key_rotation_config.rotation_interval_days);
    ...
    pub async fn batch_re_encrypt_tokens(&self, encrypted_tokens: Vec<&[u8]>) -> Result<Vec<Vec<u8>>> {
...
mod tests {
    ...
    async fn test_token_pair_encryption_no_refresh() {
        ...
        let (encrypted_access, encrypted_refresh) = encryption
            .encrypt_token_pair(access_token, None)
            .unwrap();
    ...
    async fn test_key_rotation() {
        ...
        let decrypted_old = encryption.decrypt_token(&encrypted_with_key1).await.unwrap();
        ...
        let decrypted_new = encryption.decrypt_token(&encrypted_with_key2).await.unwrap();
    ...
    async fn test_legacy_format_compatibility() {
        ...
        let ciphertext = encryption.current_cipher
            .encrypt(nonce, token.as_bytes())
            .unwrap();
