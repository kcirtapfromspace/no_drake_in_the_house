use music_streaming_blocklist_backend::models::{Artist, ArtistSearchQuery, ExternalIds};
use music_streaming_blocklist_backend::services::{
    CircuitBreaker, ExternalApiService, IsniClient, MusicBrainzClient,
};
...
async fn test_external_api_service_fallback_behavior() {
    ...
    let result = service
        .search_artists_with_fallback("NonexistentArtist12345", Some(5))
        .await;
...
async fn test_musicbrainz_response_parsing() {
    ...
    assert_eq!(
        artist.external_ids.musicbrainz.unwrap(),
...
async fn test_musicbrainz_integration_with_mock() {
    ...
    // Test the search
    let results = client
        .search_artists("The Beatles", Some(10))
        .await
        .expect("MusicBrainz search should succeed with mock");

    assert_eq!(results.len(), 1);
    let artist = &results[0];
    assert_eq!(artist.canonical_name, "The Beatles");
    assert_eq!(
        artist.external_ids.musicbrainz,
        Some("b10bbbfc-cf9e-42e0-be17-e2c3e1d2600d".to_string())
    );
    assert_eq!(
        artist.external_ids.isni,
        Some("0000000123456789".to_string())
    );
    ...
    let secondary_alias = artist
        .aliases
        .iter()
        .find(|a| a.name == "Fab Four")
        .unwrap();
...
async fn test_entity_resolution_with_mock_external_apis() {
    ...
    use music_streaming_blocklist_backend::services::external_apis::MusicBrainzClient;
    use music_streaming_blocklist_backend::services::{
        EntityResolutionService, ExternalApiService,
    };
    use wiremock::matchers::{method, path, query_param};
    use wiremock::{Mock, MockServer, ResponseTemplate};
    ...
    assert_eq!(
        result.artist.external_ids.musicbrainz,
        Some("3aa81c21-3c3b-4c8e-8f5e-5c5c5c5c5c5c".to_string())
    );
    ...
    let legal_name_alias = result
        .artist
        .aliases
        .iter()
        .find(|a| a.name == "Aubrey Graham")
        .unwrap();
...
async fn test_musicbrainz_rate_limiting_with_mock() {
    ...
    Mock::given(method("GET"))
        .and(path("/ws/2/artist"))
        .respond_with(
            ResponseTemplate::new(200).set_body_string(r#"{"artists":[],"count":0,"offset":0}"#),
        )
        .mount(&mock_server)
        .await;
...
async fn test_musicbrainz_alias_confidence_scoring() {
    use music_streaming_blocklist_backend::services::external_apis::{
        MusicBrainzAlias, MusicBrainzArtist, MusicBrainzClient,
    };
    ...
    let primary_alias = artist
        .aliases
        .iter()
        .find(|a| a.name == "Primary Alias")
        .unwrap();
    ...
    let secondary_alias = artist
        .aliases
        .iter()
        .find(|a| a.name == "Secondary Alias")
        .unwrap();
    assert_eq!(secondary_alias.confidence, 0.9); // Artist name type but not primary

    let other_alias = artist
        .aliases
        .iter()
        .find(|a| a.name == "Other Type Alias")
        .unwrap();
...
async fn test_concurrent_entity_resolution_with_mock() {
    ...
    for result_set in results {
        ...
        println!(
            "Concurrent resolution completed with {} results",
            result_set.len()
        );
    }
}
...
async fn test_external_api_service_comprehensive_fallback() {
    ...
    let result = service
        .search_artists_with_fallback("VerySpecificNonexistentArtistName12345", Some(3))
        .await;
