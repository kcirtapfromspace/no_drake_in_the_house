...
async fn test_user_registration_and_login() {
    ...
    let user = auth_service.register_user(register_request.clone()).await.unwrap();
...
async fn test_password_hashing_strength() {
    ...
    if parts.len() >= 3 {
        ...
        assert!(cost >= 12, "Password hash cost should be at least 12, got {}", cost);
    }
}
...
async fn test_refresh_token_functionality() {
    ...
    // Test refresh token
    let new_tokens = auth_service.refresh_token(&initial_tokens.refresh_token).await.unwrap();
    ...
    let result = auth_service.refresh_token(&initial_tokens.refresh_token).await;
