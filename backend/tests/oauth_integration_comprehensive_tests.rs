...
use wiremock::{MockServer, Mock, ResponseTemplate, matchers::{method, path, header, body_string_contains}};
use serde_json::json;

use music_streaming_blocklist_backend::models::oauth::{
    OAuthProviderType, OAuthUserInfo, OAuthTokens, OAuthFlowResponse, 
    OAuthAccount, AccountLinkRequest
};
    ...
    async fn setup_oauth_flow_mocks(&self) {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(ResponseTemplate::new(302)
                .insert_header("Location", "http://localhost:3000/callback?code=mock_auth_code&state=test_state"))
            .mount(&self.server)
            .await;
...
async fn create_test_user(auth_service: &AuthService) -> music_streaming_blocklist_backend::models::user::User {
...
async fn test_oauth_account_linking_integration() {
    ...
    let flow_result = auth_service
        .initiate_oauth_flow(OAuthProviderType::GitHub, "http://localhost:3000/callback".to_string())
        .await;
            ...
            match link_result {
                Ok(_) => {
                    ...
                    match accounts_result {
                        Ok(accounts) => {
                            let github_account = accounts.iter()
...
async fn test_oauth_account_unlinking_integration() {
    ...
    let flow_result = auth_service
        .initiate_oauth_flow(OAuthProviderType::Google, "http://localhost:3000/callback".to_string())
        .await;
    ...
    if let Ok(flow_response) = flow_result {
        ...
        if auth_service.link_oauth_account(user.id, link_request).await.is_ok() {
            // Now test unlinking
            ...
            match unlink_result {
                Ok(_) => {
                    ...
                    match accounts_result {
                        Ok(accounts) => {
                            let google_account = accounts.iter()
...
async fn test_oauth_provider_not_configured() {
    ...
    let result = auth_service
        .initiate_oauth_flow(OAuthProviderType::Apple, "http://localhost:3000/callback".to_string())
        .await;
    ...
    if let Ok(flow_response) = flow_result {
        ...
        if auth_service.link_oauth_account(user.id, link_request).await.is_ok() {
            ...
            if let Ok(flow_response2) = flow_result2 {
                ...
                let duplicate_result = auth_service.link_oauth_account(user.id, link_request2).await;
    ...
    for _ in 0..10 {
        ...
        match flow_result {
            Ok(flow_response) => {
                ...
                assert!(states.insert(flow_response.state.clone()), "State parameter should be unique");
                
                // State should be sufficiently long for security
                assert!(flow_response.state.len() >= 16, "State parameter should be at least 16 characters");
            }
            Err(_) => {
            ...
            match accounts_result {
                Ok(accounts) => {
                    if let Some(account) = accounts.first() {
                        ...
                        let encrypted_str = String::from_utf8_lossy(&account.access_token_encrypted);
...
async fn test_oauth_flow_performance() {
    ...
    match flow_result {
        Ok(_) => {
            ...
            assert!(duration.as_millis() < 100, "OAuth flow initiation took {}ms, should be under 100ms", duration.as_millis());
        ...
        Err(_) => {
            ...
            assert!(duration.as_millis() < 50, "OAuth error response took {}ms, should be under 50ms", duration.as_millis());
...
async fn test_oauth_concurrent_operations() {
    ...
    println!("Concurrent OAuth operations: {} succeeded, {} failed", success_count, error_count);
...
async fn test_oauth_provider_health_monitoring() {
    ...
    for provider in [OAuthProviderType::Google, OAuthProviderType::Apple, OAuthProviderType::GitHub] {
        ...
        println!("OAuth provider {} health status: {:?}", provider, health_status);
        
        // Test circuit breaker status
        let circuit_status = auth_service.get_oauth_circuit_breaker_status(&provider).await;
        println!("OAuth provider {} circuit breaker status: {:?}", provider, circuit_status);
    }
}
...
async fn test_oauth_security_monitoring() {
    ...
    let google_stats = auth_service.get_oauth_security_stats(Some(OAuthProviderType::Google), 24).await;
...
async fn test_oauth_token_status_monitoring() {
    ...
    match token_statuses {
        Ok(statuses) => {
            assert!(statuses.is_empty(), "User without OAuth accounts should have no token statuses");
    ...
    match account_health {
        Ok(health_statuses) => {
            assert!(health_statuses.is_empty(), "User without OAuth accounts should have no health statuses");
...
async fn test_oauth_expired_token_refresh() {
    ...
    match refresh_result {
        Ok(refreshed_providers) => {
            assert!(refreshed_providers.is_empty(), "User without OAuth accounts should have no tokens to refresh");
